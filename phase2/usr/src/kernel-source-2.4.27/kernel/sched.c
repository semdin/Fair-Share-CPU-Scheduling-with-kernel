...
int uid_array[2000]; // UID_MAX, kullanılan maksimum UID değeridir.
...
repeat_schedule:
	...
	if (unlikely(!c)) {
		...
		// Yeni bir uid dizisi oluştur
		
		memset(uid_array, 0, sizeof(uid_array));

		// Aktif processleri dolaş ve UID'leri say
		for_each_task(p) {
			if (p->state == 0) {
				uid_array[p->uid]++; // UID'yi say
			}
		}

		// UID'lere göre counter değerlerini hesapla
		for_each_task(p) {
			if (uid_array[p->uid]) {
				//p->counter = (p->counter >> 1) + 6*60 /  uid_array[p->uid];
